...
def keyboardInterruptHandler(signal, frame):
	...
	warped_part = (cam.imgRight.shape[0],cam.imgRight.shape[1]*cam.right_area_crop_from,3)
	shape_left_img = (cam.imgLeft.shape[0],cam.imgRight.shape[1]*cam.left_area_crop_to,3)
...
def findCropArea():
	...
	imageB = cam.imgLeft[:, 0:int(w*cam.left_area_crop_to)]
	imageA = cam.imgRight[:, int(w*cam.right_area_crop_from):w] 
	h,w,_ = imageA.shape
	...
	result = result[upper_y_crop:lower_y_crop:,:x_crop]
...
if (__name__=="__main__"):
	...
	while(not rospy.is_shutdown()):
		...
		if (stitched_img.all() != None):
			# stitched_img = cv2.cvtColor(stitched_img,cv2.COLOR_BGR2HSV)
			# clahe = cv2.createCLAHE(clipLimit=5)
			# final_img = clahe.apply(stitched_img[:,:,2])
			# cv2.imshow('value',stitched_img[:,:,2])
			# cv2.imshow('clahe',final_img)
			# stitched_img[:,:,2] = final_img
			# stitched_img = cv2.cvtColor(stitched_img,cv2.COLOR_HSV2BGR)
			...
			print(stitched_norm.shape)
			left_img_cropped_width = int(cam.imgLeft.shape[1]*cam.left_area_crop_to)
...
#============================ beside sobel we also consider the rotation between 2 camera. Ideally it should be around (0,0,0)
			...
			for rotation in rotations:
				...
				print(r)
				if(np.linalg.norm(r-np.array([0,0,0])) <8):
					satisfying_orientation=True
					break

			if 	(np.sum(crop_sobel) < h_stitched*0.2*0.9 and satisfying_orientation):
					print("high quality image",good_img_num)
					good_img_num = good_img_num +1
	...
	print("number of good stitch img {}".format(good_img_num))
	...
	lower_y_crop,upper_y_crop,x_crop = findCropArea()
	data = dict(homography_matrix=H.tolist(),crop_area_x = [0,x_crop],crop_area_y = [upper_y_crop,lower_y_crop])
	if(H is not None):
		with open(output_config_file, 'w+') as outfile:
			yaml.dump(data, outfile, default_flow_style=False)
